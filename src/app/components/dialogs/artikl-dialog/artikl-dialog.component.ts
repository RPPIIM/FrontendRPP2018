...
export class ArtiklDialogComponent implements OnInit {
  ...
  public add(): void {
    ...
    this.snackBar.open('Uspešno dodat artikl: ' + this.data.naziv, 'U redu', {duration: 2500});
  }

  public update(): void {
    this.artiklService.updateArtikl(this.data);
    this.snackBar.open('Uspešno modifikovan artikl: ' + this.data.id, 'U redu', {duration: 2500});
  }

  public delete(): void {
    this.artiklService.deleteArtikl(this.data.id);
    this.snackBar.open('Uspešno obrisan artikl: ' + this.data.id, 'U redu', {duration: 2500});
  }
